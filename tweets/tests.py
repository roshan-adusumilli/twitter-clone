...
class TweetTestClass(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='123', password='456')
        self.userb = User.objects.create_user(username='789', password='101112')
    ...
    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='456')
        return client 
